		...
		OB.OB_HX_OUTCOME_DT, 
		OB.OB_HX_GEST_AGE, 
		MIN_WEIGHT.BMI as STARTING_BMI,
		ENC.BMI as ENDING_BMI,
		...
		Inner Join (Select Distinct 
							...
							Left Join PATIENT PAT on PAT_ENC.PAT_ID=PAT.PAT_ID
		...
		Inner Join (Select Distinct 
							...
							ENC.HEIGHT,
							ENC3.OB_TOTAL_WT_GAIN/16 as OB_TOTAL_WGHT_GAIN
							From PAT_ENC ENC 
							Left Join PAT_ENC_3 ENC3 on ENC.PAT_ENC_CSN_ID=ENC3.PAT_ENC_CSN
							Left Join PATIENT on ENC.PAT_ID=PATIENT.PAT_ID
							Left Join OB_HISTORY OB on ENC.PAT_ID=OB.PAT_ID
				...
				) as MIN_WEIGHT on PATIENT.PAT_MRN_ID=MIN_WEIGHT.PAT_MRN_ID
		Where 
		ENC.CONTACT_DATE >= CURRENT_DATE-700 --Encounters in the last 700 days
		And ENC.CONTACT_DATE < CURRENT_DATE --Encounters before today
		And OB.OB_HX_OUTCOME_DT > CURRENT_DATE-365 --OB Outcome Date in the last year
		And OB.OB_HX_OUTCOME_DT > ENC.CONTACT_DATE --Encounter Date is less than the Outcome Date
		And OB.OB_HX_OUTCOME_DT is not null --Patient had a baby
		And ENC3.OB_TOTAL_WT_GAIN is not null --Weight Gain Recorded
		And ENC.BMI is not null
		And STARTING_BMI >= '30'
		And ENC.PAT_ID  not in (Select Distinct PAT_ID From PATIENT_TYPE Where PATIENT_TYPE_C='1214') --Test Patient Elimination
		And ENC.ENC_TYPE_TITLE in any ('OFFICE VISIT','HOSPITAL ENCOUNTER','OB OFFICE VISIT','PROCEDURE ONLY') --Visit Type
